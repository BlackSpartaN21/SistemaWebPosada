...
<div class="container-fluid mt-4">
  ...
  <div class="table-responsive">
    ...
    <table id="clientesTable" class="table table-bordered table-hover table-striped" data-page-length="25">
      <thead>
        <tr>
          <!-- <th>ID</th> ELIMINADO -->
          <th>Documento</th>
      ...
      <tbody>
        <?php foreach ($reservas as $reserva): ?>
          <tr>
            <!-- <td><?= (int)$reserva['id_reserva'] ?></td> ELIMINADO -->
            ...
            <?php
              $tsLlegada   = strtotime($reserva['fecha_llegada']);
              $isoLlegada  = date('Y-m-d H:i:s', $tsLlegada);
              $dispLlegada = date('d-m-Y h:i A', $tsLlegada);

              $tsSalida   = strtotime($reserva['fecha_salida']);
              $isoSalida  = date('Y-m-d H:i:s', $tsSalida);
              $dispSalida = date('d-m-Y h:i A', $tsSalida);
            ?>
            <td data-order="<?php echo $isoLlegada; ?>" data-search="<?php echo $isoLlegada; ?>">
              <?php echo $dispLlegada; ?>
            </td>
            <td data-order="<?php echo $isoSalida; ?>" data-search="<?php echo $isoSalida; ?>">
              <?php echo $dispSalida; ?>
            </td>
...
<script>
  $(document).ready(function () {
    $('#clientesTable').DataTable({
      ...
      responsive: true
      // Si quieres ordenar por "Fecha Llegada" al cargar:
      // order: [[6, 'desc']]
    });
  });
</script>
